...
function initFloatingCapsuleHeader() {
  ...
  header.innerHTML = `
    <div class="capsule-nav">
      <!-- STRICT LOGO COMPLIANCE: Only assets/logo.png is used. No SVG fallbacks. -->
      ...
      <nav class="capsule-links">
        ...
        <a href="home.html#packages">Packages</a>
  ...
  style.innerHTML = `
    ...
    .capsule-nav {
      ...
      box-shadow: 0 10px 30px -5px rgba(0, 0, 0, 0.08), inset 0 1px 0 rgba(255, 255, 255, 1);
    }
    ...
    .brand-img-logo { width: 100%; height: 100%; object-fit: contain; filter: drop-shadow(0 4px 6px rgba(15, 98, 254, 0.2)); }
    .logo-text { font-size: 20px; font-weight: 800; color: #0B101E; letter-spacing: -0.5px; }
    .logo-text span { color: #0F62FE; }
    
    .capsule-links { display: flex; gap: 28px; font-size: 14px; font-weight: 600; color: #64748B; }
    .capsule-links a { text-decoration: none; color: inherit; transition: color 0.2s; }
    .capsule-links a.active, .capsule-links a:hover { color: #0F62FE; }
    ...
    @media (max-width: 767px) {
      #floating-capsule-header { top: 10px; padding: 0 10px; }
      .capsule-nav { padding: 10px 16px; border-radius: 24px; }
...
function initMobileBottomNav() {
  ...
  nav.innerHTML = `
    ...
    <a href="home.html#packages" class="nav-item">
...
function initFloatingSupport() {
  ...
  style.innerHTML = `
    ...
    @media (max-width: 767px) {
      #floating-support-btn { bottom: 90px; right: 16px; } /* Lifted above mobile dock */
...
// ==========================================
// 4. OFFLINE DETECTOR (Sleek Slide-down Banner)
// ==========================================
  ...
  window.addEventListener('offline', () => {
    ...
    if (!bar) {
      ...
      bar.innerHTML = `
        ...
        <span>Network disconnected. Operating in offline demo mode.</span>
      `;
  ...
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => {
      navigator.serviceWorker.register('/sw.js').catch(() => {
        // Silent catch for PWA environments without valid sw.js configured yet
      });
    });
  }
}